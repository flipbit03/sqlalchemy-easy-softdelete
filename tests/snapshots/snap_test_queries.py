...
snapshots['test_ensure_aggregate_from_multiple_table_deletion_works_active_object_count 1'] = 'SELECT count(*) AS count_1 FROM sdchild JOIN sdparent ON sdparent.id = sdchild.parent_id WHERE sdchild.deleted_at IS NULL AND sdparent.deleted_at IS NULL'

snapshots['test_ensure_aggregate_from_multiple_table_deletion_works_active_object_count 2'] = 1

snapshots['test_ensure_table_with_inheritance_works 1'] = 'SELECT sdbaserequest.base_field, sdbaserequest.deleted_at, sdbaserequest.id AS id_1, sdbaserequest.request_type, sdderivedrequest.derived_field, sdderivedrequest.id FROM sdbaserequest JOIN sdderivedrequest ON sdbaserequest.id = sdderivedrequest.id WHERE sdbaserequest.deleted_at IS NULL'
...
snapshots['test_query_single_table 1'] = 'SELECT sdchild.deleted_at, sdchild.id, sdchild.parent_id FROM sdchild WHERE sdchild.deleted_at IS NULL'
...
snapshots['test_query_union_sdchild_core 1'] = 'SELECT sdchild.id, sdchild.parent_id, sdchild.parent_id FROM sdchild WHERE sdchild.deleted_at IS NULL UNION SELECT sdchild.id FROM sdchild WHERE sdchild.deleted_at IS NULL'

snapshots['test_query_with_join 1'] = 'SELECT sdchild.deleted_at, sdchild.id, sdchild.parent_id FROM sdchild JOIN sdparent ON sdparent.id = sdchild.parent_id WHERE sdchild.deleted_at IS NULL AND sdparent.deleted_at IS NULL'
...
snapshots['test_query_with_more_than_one_join 1'] = 'SELECT sdparent.deleted_at, sdparent.id FROM sdparent JOIN sdchild ON sdparent.id = sdchild.parent_id JOIN sdchildchild ON sdchild.id = sdchildchild.child_id WHERE sdparent.id > :id_1 AND sdparent.deleted_at IS NULL AND sdchild.deleted_at IS NULL AND sdchildchild.deleted_at IS NULL'

snapshots['test_query_with_table_clause_as_table 1'] = 'SELECT id FROM sdderivedrequest'

snapshots['test_query_with_text_clause_as_table 1'] = 'SELECT id FROM sdderivedrequest'
